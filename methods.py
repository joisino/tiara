...
class Solver():
    def __init__(self, env, budget, seed, verbose=False):
        self.f = env.f
...
class Tiara(Solver):
    def __init__(self, env, budget, seed=0, verbose=False, budget_ini=1, word_embedding=None, lam=1.0, alpha=0.01, uncase=True, split='bar', aggregation='mean'):
        super(Tiara, self).__init__(env, budget, seed, verbose)
        self.uncase = uncase
        self.split = split
    ...
    def optimize(self):
        ...
        for loop in range(self.budget_ini):
            item, tag = self.random_draw()
            self.update(item, tag, loop)
            self.verbose_print(loop)
        for loop in range(self.budget - self.budget_ini):
            ...
            for tag_id in tags_sorted:
                ...
                if item is not None:
                    break
            self.update(item, self.tags[tag_id], self.budget_ini + loop)
            self.verbose_print(self.budget_ini + loop)
        self.verbose_print_end()
        return self.bestitem, self.bestscore


class TiaraS(Tiara):
    def __init__(self, env, budget, seed=0, verbose=False, budget_ini=1, word_embedding=None, lam=1.0, alpha=1.0):
        super(TiaraS, self).__init__(env, budget, seed, verbose, budget_ini, word_embedding, lam, alpha)
...
class Bandit(Solver):
    def __init__(self, env, budget, seed=0, adaptive=False, verbose=False, **kwargs):
        super(Bandit, self).__init__(env, budget, seed, verbose)

        self.sums = np.zeros(len(self.tags), dtype=np.float)
        self.nums = np.zeros(len(self.tags), dtype=np.int)

        self.adaptive = adaptive

    def update(self, item, tag, loop):
        if item is None:
            # in case item ran out
            val = 0
        else:
            val = self.f(item)
        if self.adaptive and item is not None:
            for t in self.item_to_tag(item):
                if t not in self.tags:
                    self.tags.append(t)
                    self.sums = np.append(self.sums, 0)
                    self.nums = np.append(self.nums, 0)
    ...
    def optimize(self):
        np.random.seed(self.seed)
        for loop in range(self.budget):
            tag = self.bandit_draw()
            item = self.draw(tag)
            self.update(item, tag, loop)
            self.verbose_print(loop)
        self.verbose_print_end()
...
class EPSGreedy(Bandit):
    def __init__(self, env, budget, seed=0, adaptive=False, verbose=False, eps=0.01, **kwargs):
        super(EPSGreedy, self).__init__(env, budget, seed, adaptive, verbose)

        self.eps = eps

    def bandit_draw(self):
        zeropos = np.where(self.nums == 0)[0]
        if len(zeropos) > 0:
            return self.tags[zeropos[0]]
        if np.random.rand() < self.eps:
            return np.random.choice(self.tags)
        means = self.sums / self.nums
        return self.tags[np.argmax(means)]


class UCB(Bandit):
    def __init__(self, env, budget, seed=0, adaptive=False, verbose=False, alpha=1, **kwargs):
        super(UCB, self).__init__(env, budget, seed, adaptive, verbose, **kwargs)
    ...
    def bandit_draw(self):
        ...
        if len(zeropos) > 0:
            return self.tags[zeropos[0]]
        scores = self.sums / self.nums + self.alpha * np.sqrt(1 / self.nums)
        return self.tags[np.argmax(scores)]


class RandomQuery(Solver):
    def __init__(self, env, budget, seed=0, verbose=False, **kwargs):
        super(RandomQuery, self).__init__(env, budget, seed, verbose)
